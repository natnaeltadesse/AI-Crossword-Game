...
def submit_word():
    ...
    # Check if the word is already answered
    if word in answered_words:
        messagebox.showerror('Error', 'The word is already answered.')
        return
    ...
    # Add the word to the answered words set
    answered_words.append(word)

    # Change the clue label background color to green
    clue_labels[word.lower()].config(bg='green')
